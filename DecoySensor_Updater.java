 ...
 */
public class DecoySensor_Updater extends ViewableAtomic {
	...
	public DecoySensor_Updater() {
		this("Sensor_Updater");
	}

	// Add Parameterized Constructors
	public DecoySensor_Updater(String name) {
		super(name);
		...
		for (int i = 0; i < x.size(); i++) {
			if (phaseIs("UPDATE")) {
				if (messageOnPort(x, "move_result", i)) {
					if(x.getValOnPort("move_result", i).eq("decoy")) {
